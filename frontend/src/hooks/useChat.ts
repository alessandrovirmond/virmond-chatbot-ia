...
export const useChat = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 1, text: "Olá! Front-End e Back-End 100% organizados. Pronto para a IA!", isUser: false }
  ]);
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    setIsLoading(true); // Trava o botão enquanto espera o C#
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, text: "❌ Ops, ocorreu um erro de conexão.", isUser: false }
      ]);
    } finally {
      setIsLoading(false); 
    }
